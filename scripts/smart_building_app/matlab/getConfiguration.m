...
function [ret_val] = getConfiguration(argType)
    if(argType == 1)
        ret_val = 'D:\Projects\EdgeCloudSimulator\sim_results\';    %Path to Edge cloud Simulation results
    elseif(argType == 2)
    ...
    elseif(argType == 7)
        ret_val = {'MOBILE_PROCESSING_SCENARIO','SINGLE_TIER','TWO_TIER','TWO_TIER_WITH_EO'};   %Simulation Scenarios
    elseif(argType == 8)
        ret_val = {'ONLY_EDGE','ONLY_MOBILE','HYBRID'}; %Orchestrator policies
    elseif(argType == 9)
