function [] = plotGenericResult(rowOfset, columnOfset, yLabel, appType, calculatePercentage)
    ...
    policyType = getConfiguration(8);
    legends = getConfiguration(9);
    numOfMobileDevices = (endOfMobileDeviceLoop - startOfMobileDeviceLoop)/stepOfMobileDeviceLoop + 1;

    pos=getConfiguration(10);
    
    all_results = zeros(numOfSimulations, size(scenarioType,3), numOfMobileDevices);
    min_results = zeros(size(scenarioType,3), numOfMobileDevices);
    max_results = zeros(size(scenarioType,3), numOfMobileDevices);
    
    for s=1:numOfSimulations
        for i=1:size(scenarioType,2)
            for k=1:size(policyType,2)
                for j=1:numOfMobileDevices
                    try
                        mobileDeviceNumber = startOfMobileDeviceLoop + stepOfMobileDeviceLoop * (j-1);
                        filePath = strcat(folderPath,'\ite',int2str(s),'\SIMRESULT_',char(scenarioType(i)),'_',char(policyType(k)),'_',int2str(mobileDeviceNumber),'DEVICES_',appType,'_GENERIC.log');
            ...
            end %end of orchestrator policy loop
        end %end of simulator scenario types loop
