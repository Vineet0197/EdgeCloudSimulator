...
public class SmartBuildingAppDeviceManager extends MobileDeviceManager {
    private static final int BASE = 100000; //start from base in order not to conflict cloudsim tag!

    private static final int REQUEST_RECEIVED_BY_EDGE_DEVICE = BASE + 1;
    private static final int REQUEST_RECEIVED_BY_SENSOR = BASE + 2;
    private static final int RESPONSE_RECEIVED_BY_SENSOR = BASE + 3;

    private int taskIdCounter=0;

    public SmartBuildingAppDeviceManager() throws Exception{
    }
        ...
        if(task.getAssociatedDatacenterId() == SimSettings.GENERIC_EDGE_DEVICE_ID){
            ...
            if(delay > 0)
            {
                ...
                if(task.getSubmittedLocation().getServingWlanId() == currentLocation.getServingWlanId())
                {
                    ...
                    schedule(getId(), delay, RESPONSE_RECEIVED_BY_SENSOR, task);
                }
                else
                {
    ...
    protected void processOtherEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            case REQUEST_RECEIVED_BY_SENSOR:
            {
                Task task = (Task) ev.getData();
            ...
            case RESPONSE_RECEIVED_BY_SENSOR:
            {
                Task task = (Task) ev.getData();
    ...
    public void submitTask(TaskProperty edgeTask) {
        ...
        if(nextHopId == SimSettings.GENERIC_EDGE_DEVICE_ID){
            ...
            nextEvent = REQUEST_RECEIVED_BY_SENSOR;
