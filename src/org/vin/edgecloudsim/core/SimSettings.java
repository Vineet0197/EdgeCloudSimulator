...
public class SimSettings {
    ...
    private Document edgeDeviceDoc = null;
    ...
    private int MIN_NUM_OF_SENSORS;
    private int MAX_NUM_OF_SENSORS;
    private int SENSOR_DEVICE_COUNTER_SIZE;
    ...
    public boolean initialize(String propertiesFile, String edgeDevicesFile, String applicationsFile){
        ...
        try {
            ...
            MIN_NUM_OF_SENSORS = Integer.parseInt(prop.getProperty("min_number_of_sensors"));
            MAX_NUM_OF_SENSORS = Integer.parseInt(prop.getProperty("max_number_of_sensors"));
            SENSOR_DEVICE_COUNTER_SIZE = Integer.parseInt(prop.getProperty("sensor_device_counter_size"));
    ...
    /**
     * returns the minimum number of the sensors used in the simulation
     */
    public int getMinNumOfMobileDev()
    {
        return MIN_NUM_OF_SENSORS;
    }

    /**
     * returns the maximum number of the sensors used in the simulation
     */
    public int getMaxNumOfMobileDev()
    {
        return MAX_NUM_OF_SENSORS;
    }

    /**
     * returns the number of increase on sensor edge devices
     * while iterating from min to max mobile device
     */
    public int getMobileDevCounterSize()
    {
        return SENSOR_DEVICE_COUNTER_SIZE;
    }
    ...
    private void parseEdgeDevicesXML(String filePath)
    {
        try {
            ...
            DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
            DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
            edgeDeviceDoc = dBuilder.parse(devicesFile);
            edgeDeviceDoc.getDocumentElement().normalize();

            NodeList datacenterList = edgeDeviceDoc.getElementsByTagName("datacenter");
